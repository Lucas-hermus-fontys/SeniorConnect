...
public class GroupChatService
{
    ...
    public List<CollaborativeSpace> GetGroupChatsByUserId(int userId)
    {
        ...
        users.ForEach(user => groupChats.FirstOrDefault(o => o.Id == user.CollaborativeSpaceId)?.AddCollaborativeSpaceUser(user));
        messages.ForEach(message => groupChats.FirstOrDefault(o => o.Id == message.CollaborativeSpaceId)?.AddCollaborativeSpaceMessage(message));
    ...
    public CollaborativeSpace GetGroupChatById(int id)
    {
        ...
        messages.ForEach(message => message.User = users.FirstOrDefault(user => user.Id == message.UserId));
        messages.ForEach(message => groupchat.AddCollaborativeSpaceMessage(message));
